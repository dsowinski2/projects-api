...
from backend.src.utils.types import ProjectType
...
@projects_router.get("/")
def get_projects(
    controller: ProjectsController = Depends(controler_builder),
) -> List[ProjectType]:
...
def get_project(
    project_id, controller: ProjectsController = Depends(controler_builder)
):
...
def patch_project(
    project_id,
    project_data: ProjectUpdateType,
    controller: ProjectsController = Depends(controler_builder),
):
